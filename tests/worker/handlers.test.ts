...
const runSimilarPhotoJob = vi.fn();
const clipInstance = { embedImage: vi.fn() };
...
vi.mock("@/lib/duplicates/similar", () => ({ runSimilarPhotoJob }));
vi.mock("@/lib/classification/clip", () => ({
  ClipService: class {
    embedImage = clipInstance.embedImage;
  },
}));
...
beforeEach(() => runSimilarPhotoJob.mockReset());
...
describe("worker dispatch", () => {
  it("dispatches find-similar jobs with a local CLIP service", async () => {
    ...
    expect(runSimilarPhotoJob).toHaveBeenCalledTimes(1);
    const [context, jobId] = runSimilarPhotoJob.mock.calls[0]!;
